...
int CWindowsEngine::Init(FWinMainCommandParameters InParameters)
{
    ...
    RenderingEngine->SetMainWindowsHandle(MainWindowsHandle);//设置Windows句柄

    RenderingEngine->Init(InParameters);//引擎初始化

    CWorld* World = CreateObject<CWorld>(new CWorld());//创建世界对象
